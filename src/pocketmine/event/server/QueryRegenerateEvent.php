...
class QueryRegenerateEvent extends ServerEvent {
	...
	public function __construct(Server $server){
		...
		if($server->isDServerEnabled() and $server->dserverConfig["queryMaxPlayers"]){
			$maxPlayers = $server->dserverConfig["queryMaxPlayers"];
		}elseif($server->isDServerEnabled() and $server->dserverConfig["queryAllPlayers"]){
			$maxPlayers = $server->getDServerMaxPlayers();
		}else{
			$maxPlayers = $server->getMaxPlayers();
		}

		if($server->isDServerEnabled() and $server->dserverConfig["queryPlayers"]){
			$onlinePlayers = $server->getDServerOnlinePlayers();
		}else{
			$onlinePlayers = count($this->players);
		}

		$this->gametype = ($server->getGamemode() & 0x01) === 0 ? "SMP" : "CMP";
		$this->version = $server->getVersion();
		$this->server_engine = "{$server->getName()} {$server->getPocketMineVersion()} ({$server->getCodename()})";
		$this->map = $server->getDefaultLevel() === null ? "unknown" : $server->getDefaultLevel()->getName();
		$this->numPlayers = $onlinePlayers;
