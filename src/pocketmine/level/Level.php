...
class Level implements ChunkManager, Metadatable{
      ...
      if($this->spawnTicks <= 0){
        ...
        foreach($this->getPlayers() as $player){
          ...
          if(count($entities) < 10){
            $x = mt_rand($player->getX() - 20, $player->getX() + 20);
  					$z = mt_rand($player->getZ() - 20, $player->getZ() + 20);
  					$pos = new Vector3($x, $player->getY(), $z);
  					$pos = $this->getSafeSpawn($pos);
  					$pos = new Position($pos->x, $pos->y, $pos->z, $this);
  					$mobs = [];
  					$time = $this ? $this->getTime() % Level::TIME_FULL : Level::TIME_NIGHT;
  					if(($this->getServer()->getConfigString("level-name", "world") == $this->getName()) and ($time > Level::TIME_NIGHT) and ($time < Level::TIME_SUNRISE)){
  						$mobs[] = "Zombie";
  						$mobs[] = "ZombieVillager";
  						$mobs[] = "Creeper";
  						$mobs[] = "Spider";
  						$mobs[] = "Skeleton";
  						$mobs[] = "Witch";
  					}elseif($this->getName() == $this->getServer()->netherName){
              if(mt_rand(0, 100) < 10){
                $mobs[] = "Ghast";
              }
  						$mobs[] = "PigZombie";
  					}elseif(($this->getName() == $this->getServer()->getConfigString("level-name", "world")) and (($time < Level::TIME_NIGHT) or ($time > Level::TIME_SUNRISE))){
  						$mobs[] = "Pig";
  						$mobs[] = "Sheep";
  						$mobs[] = "Cow";
  						$mobs[] = "Chicken";
              ...
              if(mt_rand(0, 100) < 5){
                $mobs[] = "Horse";
  					...
  					}elseif($this->getName() == $this->getServer()->enderName){
              $mobs[] = "Enderman";
            }

  					if(count($mobs) > 0){
              if($this->getChunk($pos->x >> 4, $pos->z >> 4) == null) return;
              $id = array_rand($mobs);
              $nbt = new CompoundTag("", [
                "Pos" => new ListTag("Pos", [
                  new DoubleTag("", $pos->x),
              ...
              if(!($t->distance($s) <= $this->server->getSpawnRadius())){
                $mob = Entity::createEntity($mobs[$id], $this, $nbt);
                $mob->allowMove = true;
                $mob->spawnToAll();
              }
  					}
    ...
    public function getCollisionBlocksArrow(AxisAlignedBB $bb){
  ...
  public function getBlock(Vector3 $pos, bool $cached = true, bool $addToCache = true) : Block{
    ...
    public function addEntityMovementAll($viewers, $entityId, $x, $y, $z, $yaw, $pitch, $headYaw = null, $isPlayer = false){
      if (empty($viewers)) {
  			return;
  		}
