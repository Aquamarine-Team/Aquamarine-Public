...
use pocketmine\entity\{Entity, BlazeFireball, Boat, Car, Item as DroppedItem, EnderCrystal, EnderPearl, FallingSand, FishingHook, Lightning, Minecart, MinecartChest, MinecartCommandBlock, MinecartHopper, MinecartTNT, Painting, PrimedTNT, XPOrb, Human, EnderDragon, Endermite, Guardian, PolarBear, Shulker, Slime, SkeletonHorse, Squid, Vindicator, Witch, WitherSkeleton, ZombieHorse, ElderGuardian, BlueWitherSkull, BlackWitherSkull, LavaSlime};
...
use pocketmine\entity\monster\flying\{Blaze, Ghast, Vex, Wither};
use pocketmine\entity\monster\jumping\MagmaCube;
use pocketmine\entity\ThrownPotion;
use pocketmine\entity\ThrownExpBottle;
use pocketmine\entity\monster\walking\{Evoker, CaveSpider, Creeper, Enderman, Husk, IronGolem, PigZombie, Silverfish, Skeleton, SnowGolem, Spider, Stray, Wolf, Zombie, ZombieVillager};
...
§fИспользуется ядро §d{$this->getName()} {$this->getPocketMineVersion()}
§fMCPE версия: " . $this->getVersion() . "
§fИспользуется php: §e" . PHP_VERSION . "
§fOS: §6" . PHP_OS ."
§fАвтор: §bvk.com/aquamarine.team §f(§bvk.com/maruselplay§f)
...
";
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $dataPath, $pluginPath, $defaultLang = "unknown"){
		...
		try{
			...
			$this->logger->info("Загрузка настроек...");
			...
			if(file_exists(\pocketmine\PATH . "src/pocketmine/resources/aquamarine_$lang.yml")){
				$content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/aquamarine_$lang.yml");
			}else{
				$content = file_get_contents($file = \pocketmine\PATH . "src/pocketmine/resources/aquamarine_rus.yml");
			}

			if(!file_exists($this->dataPath . "aquamarine.yml")){
				@file_put_contents($this->dataPath . "aquamarine.yml", $content);
			}
	...
	private function registerEntities(){
		...
		Entity::registerEntity(BlackWitherSkull::class);
