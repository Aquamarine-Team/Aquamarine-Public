...
use pocketmine\level\sound\{MilkSound, ShearSound, ExpPickupSound};
...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer {
	...
	public $percentage = 70; // Кол-во нужных пикселей в скине
	...
	protected function checkNearEntities(){
		foreach($this->level->getNearbyEntities($this->boundingBox->expandedCopy(1, 0.5, 1), $this) as $entity){
			...
			if($entity instanceof Arrow and $entity->hadCollision){
				...
				if($entity->getBow() !== null and $entity->getBow()->hasEnchantment(Enchantment::TYPE_BOW_INFINITY))
	...
	protected function processMostRecentMovements() : void{
		...
		if($delta > 0.0001 or $deltaAngle > 1.0){
			...
			if($this->server->enderEnabled){
				if($this->isInsideOfEndPortal()){
					if($this->server->isLevelLoaded($this->server->enderName) or $this->server->loadLevel($this->server->enderName)){
						$enderLevel = $this->server->getLevelByName($this->server->enderName);
					}
					if($enderLevel instanceof Level){
						if($this->getLevel() !== $enderLevel){
							$this->teleport($this->shouldResPos = $enderLevel->getSafeSpawn());
						}else{
	...
	public function checkSkin(){
		...
		$pos = -1;
		$pixelsNeeded = (int)((100 - $this->percentage) / 100 * ($width * $height));
		for ($y = 0; $y < $height; $y++) {
			for ($x = 0; $x < $width; $x++) {
		...
		$this->close("", "На нашем сервере нельзя использовать невидимые скины");
		return false;
	}
	...
	protected function completeLoginSequence(){
		...
		if($this->getUniqueId()->getVersion() !== 3){
			$this->close("", "disconnectionScreen.invalidName", true);
	...
	public function handleDataPacket(DataPacket $packet){
      ...
      case ProtocolInfo::LOGIN_PACKET:
				...
				foreach($this->getServer()->getOnlinePlayers() as $player){
					if($player->getUniqueId()->equals(UUID::fromString($packet->clientUUID)) or strtolower($player->getName()) == strtolower($packet->username)){
						$pk = new PlayStatusPacket();
			...
			case ProtocolInfo::INTERACT_PACKET:
				...
				if($packet->action === InteractPacket::ACTION_RIGHT_CLICK){
					if($target instanceof Entity){
						if($target->canBeSheared() and $this->getInventory()->getItemInHand()->getId() === 359){
							$target->shear($this);
						}elseif($target->canBeMilked() and $this->getInventory()->getItemInHand()->getId() === 325 and $this->getInventory()->getItemInHand()->getDamage() === 0){
							$target->milk($this);
						}elseif($target->canBeIgnited() and $this->getInventory()->getItemInHand()->getId() === 259){
							$target->ignite($this);
						}elseif(($item = $this->getInventory()->getItemInHand())->getId() === 421 and !$target instanceof Player and $item->hasCustomName()){
							$item->setCount($item->getCount() - 1);
							$this->getInventory()->setItemInHand($item);
							$target->setNameTag($item->getCustomName());
							$target->setNameTagVisible();
						}
					}
					break;
			...
			case ProtocolInfo::REQUEST_CHUNK_RADIUS_PACKET:
			    ...
			    if ($radius <= 48 and $radius >= 4) {
			    	...
			    	$this->server->getNetwork()->blockAddress($this->getAddress(), 1201);
			    }
				break;
