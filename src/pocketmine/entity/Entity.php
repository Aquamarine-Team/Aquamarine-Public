...
//21 - in love (for those who can breed)
	...
	const DATA_FLAG_SHEARED = 29; //голая овца, топ люблю
...
//		self::DATA_BOUNDING_BOX_HEIGHT => [self::DATA_TYPE_FLOAT, 1.8],
	...
	public function saveNBT(){
		if(!($this instanceof Player)){
			...
			if($this instanceof BaseEntity){
				$this->namedtag->AllowMove = new StringTag("AllowMove", $this->allowMove);
			}
		}
	...
	protected function initEntity(){
		...
		if(isset($this->namedtag->AllowMove)){
			$this->allowMove = (bool) $this->namedtag["AllowMove"];
		}
	...
	public function attack($damage, EntityDamageEvent $source){
		...
		if($source->getCause() === EntityDamageEvent::CAUSE_ENTITY_ATTACK){
			if($this->distance($source->getDamager()) >= 5){
				$source->setCancelled();
			}
	 ...
	 * @return bool
	 */
	protected function checkObstruction($x, $y, $z){ // TODO: сделать мобам умных обход блоков
	...
	protected function updateMovement(bool $teleport = false){
		...
		if($diffPosition > 0.04 || $diffRotation > 2.25){
			...
			$this->level->addEntityMovementAll($this->getViewers(), $this->id, $this->x, $this->y + $this->getEyeHeight(), $this->z, $this->yaw, $this->pitch, $this->yaw, ($this instanceof Player));
		}*/
		$diffPosition = ($this->x - $this->lastX) ** 2 + ($this->y - $this->lastY) ** 2 + ($this->z - $this->lastZ) ** 2;
		$diffRotation = ($this->yaw - $this->lastYaw) ** 2 + ($this->pitch - $this->lastPitch) ** 2;
